...
2. **Session 2026-07-30 evening (autonomous, user at a concert)**: devenv committed (verovio built from source; full pipeline verified in-shell). Error tail worked: 128th notes + 4 dots + invisible-rest recovery + tuplet-offset voice padding (shared `Duration.decompose_rests/1`) — polish 8760/8918, haydn 210/210, wtc_fugues 47/48. MusicXML parser rebuilt POSITIONAL (forward/backup cursor, interleaved voices regrouped, gaps padded, graces dropped, typed refusals) — the PDMX prerequisite. OpenScore ingested via `scripts/convert_mscx.exs` (headless MuseScore batch under nix+xvfb): lieder 1341/1352 = 4.00M CC0 tokens, quartets 86/104 = 2.24M (18 mscx failed mscore conversion itself — uninvestigated; quartet p50 sequence 26K tokens). Vocab 648 (extended tuplet ratios like 6:4/2:3 kept verbatim, two appended batches). CAUTION for future vocab edits: every appended block in Vocab.tokens/0 must be FROZEN BY VALUE (see @appended_* attrs) — computing a block from Duration/Instruments lets later additions shift every id behind it; the frozen-ids test in vocab_test.exs is the guard. Corpus totals: ~44M clean-ish tokens at that point. Remaining then: PDMX download + .mxl handling.
3. **PDMX INGESTED (2026-07-31, same session)**: this machine IS the 5090 box; user approved the download. Only the needed Zenodo files pulled (mxl.tar.gz 1.9GB + subset_paths + PDMX.csv — NOT the 14.4GB total; skip pdf/data/mid archives). `Corpus` gained :mxl (zip) format, `manual:`/`subset:` source options, and parallel ingest (ordered Task.async_stream across 32 cores, offsets still deterministic). Recovery passes added: unpitched-note refusal, 8 new clefs, 256th/long durations (divisions multiplier now 512), kern 00-longa fix (was silently breve), extended tuplet ratios, pure-ratio gap padding. FINAL CORPUS: pdmx 213,727/222,820 = 154.9M tok; polish 37.7M; lieder 4.0M (99.3%); os-quartets 2.2M; NC kern tier ~2.5M — **~201M tokens, ~199M PD/CC0/CC BY**. Vocab 666, frozen-ids test pins every appended block. PDMX residual refusals: percussion/TAB/unpitched (~5.5K files, out of scope), overlapping_voice 908, unsupported_time 840, unrepresentable_offset ~250. NEXT: Phase 0.5 (task #5 — XLA_TARGET=cuda12, exla in mix.exs, devenv CUDA env, throughput bench), then Phase 1 training; consider PDMX deduplicated∩no_license_conflict subset for training splits (avoid arrangement duplicates), and sequence packing (pdmx p50 is only ~274 tokens).
